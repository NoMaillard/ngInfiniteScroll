    ...
    return {
      ...
      link: function link(scope, elem, attrs) {
        ...
        function offsetBottom(element) {
          if (!(!element[0].getBoundingClientRect || element.css('none'))) {
            return element[0].getBoundingClientRect().bottom + pageYOffset(element);
          }
          return undefined;
        }
        ...
        function defaultHandler() {
          var containerLimit = void 0;
          var elementLimit = void 0;
          if (scope.infiniteScrollReverse) {
            containerLimit = offsetTop(container);
            elementLimit = offsetBottom(elem);
          } else if (container === windowElement) {
            containerLimit = height(container) + pageYOffset(container[0].document.documentElement);
            elementLimit = offsetTop(elem) + height(elem);
          } else {
            containerLimit = height(container);
            var containerTopOffset = 0;
            if (offsetTop(container) !== undefined) {
              containerTopOffset = offsetTop(container);
            }
            elementLimit = offsetTop(elem) - containerTopOffset + height(elem);
          }

          if (useDocumentBottom) {
            elementLimit = height((elem[0].ownerDocument || elem[0].document).documentElement);
          }

          var remaining = scope.infiniteScrollReverse ? containerLimit - elementLimit : elementLimit - containerLimit;
